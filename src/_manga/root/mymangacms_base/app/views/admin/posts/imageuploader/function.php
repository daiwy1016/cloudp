...
function loadImages() {
    ...
    if(file_exists($useruploadpath)){
        ...
        for($i=count($files)-1; $i >= 0; $i--):
            ...
            if($file_style == "block") { ?>
                <div class="fileDiv"
                     onclick="showEditBar('<?php echo $image; ?>','<?php echo asset($image); ?>','<?php echo $image_height; ?>','<?php echo $count; ?>');"
                     ondblclick="showImage('<?php echo $image; ?>','<?php echo asset($image); ?>','<?php echo $image_height; ?>');"
                     data-imgid="<?php echo $count; ?>">
                    <div class="imgDiv"><img class="fileImg lazy" data-original="<?php echo asset($image); ?>"></div>
            ...
            <?php } elseif($file_style == "list") { ?>
                <div class="fullWidthFileDiv"
                    ...
                    <div class="qEditIconsDiv">
                        <img title="Delete File" src="img/cd-icon-qtrash.png" class="qEditIconsImg" onclick="window.location.href = 'imgdelete.php?img=<?php echo asset($image); ?>'">
                    </div>
