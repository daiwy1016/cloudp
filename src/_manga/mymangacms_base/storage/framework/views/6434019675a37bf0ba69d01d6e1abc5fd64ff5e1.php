...
<div class="row">
    <div class="col-md-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <i class="fa fa-magnet"></i> <?php echo e(Lang::get('messages.admin.chapter.scraper.title')); ?>

                <div class="box-tools">
                    <?php echo e(link_to_route('admin.manga.show', Lang::get('messages.admin.manga.back'), array('manga' => $manga->id), array('class' => 'btn btn-default btn-xs pull-right'))); ?>
            ...
            <div class="box-body">
                <div class="form-group">
                    ...
                    <?php elseif($settings['storage.type'] == 'mirror'): ?>
                    <div class="alert alert-info" role="alert"><?php echo e(Lang::get('messages.admin.chapter.scraper.storage-mode.mirror')); ?></div>
                    <?php else: ?>
                ...
                <div class="form-group">
                    <label for="scan-source"><?php echo e(Lang::get('messages.admin.chapter.scraper.select-source')); ?></label>
                    <select id="scan-source" class="selectpicker" data-width="100%">
                        <?php $__currentLoopData = $websites; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=>$site): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($key); ?>"> <?php echo e($site['name']); ?> </option>
                ...
                <div class="form-group scraping-mode">
                    ...
                    <select id="scraping-mode" class="selectpicker" data-width="100%">
                        ...
                        <option value="one-or-many"><?php echo e(Lang::get('messages.admin.chapter.scraper.one-many-urls')); ?></option>
                        <option value="all-chapters">Scrape all chapters from Manga page</option>
                    </select>
                </div>
                <div class="form-group one-or-many">
                    <label for="chaptersUrl"><?php echo e(Lang::get('messages.admin.chapter.scraper.one-many-urls')); ?></label>
                    ...
                    <button id="startScrapingBtn" type="button" class="btn btn-default" onclick="startScraping()">
                        <?php echo e(Lang::get('messages.admin.chapter.scraper.start')); ?>
                    ...
                    <a href="" class="btn btn-danger pull-right" data-toggle="modal" data-target="#myModal" title="After finishing scraping click here to notify users">
                        <?php echo e(Lang::get('messages.admin.chapter.scraper.notif-users')); ?>
                ...
                <br/>
                <div id="waiting" style="display: none;"><center><img src="<?php echo e(asset('images/ajax-loader.gif')); ?>" /></center></div>
                <br/>

                <div id="startScrapingProgressTotal" class="panel panel-default" style="display:none;">
                    <div class="panel-heading"><?php echo e(Lang::get('messages.admin.chapter.scraper.total-progress')); ?></div>
                    <div class="box-body">
        ...
        <div class="modal-dialog" role="document" style="z-index: 9999">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel"><?php echo e(Lang::get('messages.admin.chapter.scraper.notif-users')); ?></h4>
                </div>
                <div class="modal-body">
                    <div class="row control-group">
                        <div class="form-group col-xs-12">
                            <p><?php echo e(Lang::get('messages.admin.chapter.scraper.notif-users-message')); ?></p>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal"><?php echo e(Lang::get('messages.front.reader.close')); ?></button>
                    <button type="button" class="btn btn-primary" onclick="notifyUsers()"><?php echo e(Lang::get('messages.admin.chapter.scraper.notif-users.notify')); ?></button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        var mangaSlug = '<?php echo e($manga->slug); ?>';
        ...
        var collapseBody = '<div id="COLLAPSE-ID" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="HEADING-ID">' +
                ...
                '<thead>' +
                '<tr>' +
                "<th><?php echo e(Lang::get('messages.admin.chapter.scraper.header-number')); ?></th>" +
                "<th><?php echo e(Lang::get('messages.admin.chapter.scraper.header-scan')); ?></th>" +
                "<th><?php echo e(Lang::get('messages.admin.chapter.scraper.header-name')); ?></th>" +
                "<th><?php echo e(Lang::get('messages.admin.chapter.scraper.header-url')); ?></th>" +
                '</tr>' +
                '</thead>' +
                '<tbody></tbody>' +
                '</table>' +
        ...
        function selectSource() {
            switch ($('#scan-source').val()) {
                case "mangapanda":
                    ...
                    $('.all-chapters').hide();
                    $('.scraping-mode').show();
                    selectedWebsite = "<?php echo e($websites['mangapanda']['url']); ?>";
                ...
                case "mangareader":
                    ...
                    $('.all-chapters').hide();
                    $('.scraping-mode').show();
                    selectedWebsite = "<?php echo e($websites['mangareader']['url']); ?>";
                    $('#scraping-mode').selectpicker('val', '');
                    break;
                case "comicvn":
                    $('.one-or-many').hide();
                    $('.all-chapters').hide();
                    $('.scraping-mode').show();
                    selectedWebsite = "<?php echo e($websites['comicvn']['url']); ?>";
                    $('#scraping-mode').selectpicker('val', '');
                    break;
                case "pecinta":
                    $('.one-or-many').hide();
                    $('.all-chapters').hide();
                    $('.scraping-mode').show();
                    selectedWebsite = "<?php echo e($websites['pecinta']['url']); ?>";
                    $('#scraping-mode').selectpicker('val', '');
                    break;
                case "3asq":
                    $('.one-or-many').hide();
                    $('.all-chapters').hide();
                    $('.scraping-mode').show();
                    selectedWebsite = "<?php echo e($websites['3asq']['url']); ?>";
                    $('#scraping-mode').selectpicker('val', '');
                    break;
                case "9manga_es":
                    $('.one-or-many').hide();
                    $('.all-chapters').hide();
                    $('.scraping-mode').show();
                    selectedWebsite = "<?php echo e($websites['9manga_es']['url']); ?>";
                    $('#scraping-mode').selectpicker('val', '');
                    break;
                default:
        ...
        function startAllChaptersScraping() {
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo e(route('admin.manga.chapter.scraper.getTotalChapters')); ?>",
                data: {'mangaPageUrl': $('#mangaPageUrl').val(), '_token': '<?php echo e(csrf_token()); ?>'},
        ...
        function getChapters(k) {
            if (k < urls.length) {
                if (typeof urls[k].scanList != "undefined") {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "<?php echo e(route('admin.manga.chapter.scraper.getChapter')); ?>",
                        data: {'chapterUrl': urls[k].url, 'mangaId': mangaId, 'selectedWebsite': $('#scan-source').val(), 'chapterTitle': urls[k].title, '_token': '<?php echo e(csrf_token()); ?>'},
                        success: function (response) {
                            $('#waiting').hide();
                            $('#startScrapingProgressTotal').show();
        ...
        function getChapterImage(i, chapterId, k) {
            if (i < scanList[chapterId].length) {
                $.ajax({
                    method: "POST",
                    url: "<?php echo e(route('admin.manga.chapter.downloadImageFromUrl')); ?>",
                    data: {scanURL: scanList[chapterId][i].url, index: scanList[chapterId][i].index, 'mangaSlug': mangaSlug, 'chapterId': chapterId, _token: '<?php echo e(csrf_token()); ?>'},
                    success: function (response) {
        ...
        function saveDownloadStat() {
            $.ajax({
                method: "POST",
                url: "<?php echo e(route('admin.manga.chapter.scraper.abort')); ?>",
                data: {'mangaId': mangaId, 'bulkStatus': JSON.stringify(bulkObj), _token: '<?php echo e(csrf_token()); ?>'}
            });
        }
        ...
        function resume() {
            ...
            $.ajax({
                method: "POST",
                url: "<?php echo e(route('admin.manga.chapter.scraper.resume')); ?>",
                data: {'mangaId': mangaId, _token: '<?php echo e(csrf_token()); ?>'},
                success: function (response) {
                    noStat = true;
        ...
        function resumeDownloadingImage(k, i, url, chapterId, totalPage) {
            $.ajax({
                method: "POST",
                url: "<?php echo e(route('admin.manga.chapter.downloadImageFromUrl')); ?>",
                data: {'scanURL': url, 'index': i, 'mangaSlug': mangaSlug, 'chapterId': chapterId, '_token': '<?php echo e(csrf_token()); ?>'},
                success: function (response) {
        ...
        function startScarpingChapters(k) {
            if (k < urls.length) {
                $.ajax({
                    type: 'POST',
                    url: "<?php echo e(route('admin.manga.chapter.scraper.start')); ?>",
                    data: {'chapterUrl': urls[k], 'mangaId': mangaId, 'selectedWebsite': $('#scan-source').val(), '_token': '<?php echo e(csrf_token()); ?>'},
                    success: function (response) {
                        $('#waiting').hide();
                        $('#startScrapingProgressTotal').show();
        ...
        function retryDownloading(i, chapterId) {
            $.ajax({
                method: "POST",
                url: "<?php echo e(route('admin.manga.chapter.downloadImageFromUrl', array('mangaSlug' => $manga->slug))); ?>",
                data: {scanURL: scanList[chapterId][parseInt(i) - 1].url, index: i, 'mangaSlug': mangaSlug, 'chapterId': chapterId, '_token': '<?php echo e(csrf_token()); ?>'},
                timeout: 40000,
                success: function (response) {
        ...
        function notifyUsers() {
            $.ajax({
                method: "POST",
                url: "<?php echo e(route('admin.notify.users')); ?>",
                data: {mangaId: mangaId, 'mangaSlug': mangaSlug, '_token': '<?php echo e(csrf_token()); ?>'},
                success: function (response) {
