...
trait ClubContainer
{
	...
	public function saveClubForm( \IPS\Member\Club $club, $values )
	{
		...
		if ( !$haveId )
		{
			...
			$time = time();
			\IPS\Db::i()->insert( 'core_follow', \IPS\Db::i()->select( "MD5( CONCAT( '{$followApp};{$followArea};{$this->_id};', follow_member_id ) ) AS follow_id, '{$followApp}' AS follow_app, '{$followArea}' AS follow_area, '{$this->_id}' AS follow_rel_id, follow_member_id, follow_is_anon, '{$time}' AS follow_added, follow_notify_do, follow_notify_meta, follow_notify_freq, 0 AS follow_notify_sent, follow_visible", 'core_follow', array(
				'follow_app=? AND follow_area=? AND follow_rel_id=?', 'core', 'club', $club->id ) ) );
		}
	}
	...
	public static function clubNodes( $permissionCheck='view', $member=NULL, $where=array() )
	{
		...
		{
			$where[] = array( static::$databasePrefix . static::clubIdColumn() . ' IS NOT NULL' );
	...
	public function modPermission( $type, \IPS\Member $member, $itemClass=NULL )
	{
		...
		return parent::modPermission( $type, $member, $itemClass );
	}
