...
class _Index extends \IPS\Content\Search\Index
{
			...
			else
			{
				try
				{
					...
					else if ( $indexData['index_is_last_comment'] and $indexData['index_item_id'] )
					{
						...
						$this->url->setPath( $this->url->data[ \IPS\Http\Url::COMPONENT_PATH ] . '/content/_update_by_query' )->setQueryString( array( 'conflicts' => 'proceed', 'wait_for_completion' => 'false' ) )->request()->setHeaders( array( 'Content-Type' => 'application/json' ) )->post( json_encode( array(
							'script'	=> array(
							...
							'query'		=> array(
								'bool'		=> array(
									'must'		=> array(
										...
										array(
											'term'	=> array(
												'index_item_id' => $indexData['index_item_id']
											)
										),
									)
								)
	...
	public function updateViewCounts( $class, $id, $count )
	{
		...
		$this->url->setPath( $this->url->data[ \IPS\Http\Url::COMPONENT_PATH ] . '/content/_update_by_query' )->setQueryString( array( 'conflicts' => 'proceed', 'wait_for_completion' => 'false' ) )->request( \IPS\LONG_REQUEST_TIMEOUT )->setHeaders( array( 'Content-Type' => 'application/json' ) )->post( json_encode( array(
			'script'	=> array(
	...
	public function massUpdate( $class, $containerId = NULL, $itemId = NULL, $newPermissions = NULL, $newHiddenStatus = NULL, $newContainer = NULL, $authorId = NULL, $newItemId = NULL, $newItemAuthorId = NULL, $addAuthorToPermissions = FALSE )
	{
		try
		{
			...
			if ( $newHiddenStatus !== NULL )
			{
				...
				$this->url->setPath( $this->url->data[ \IPS\Http\Url::COMPONENT_PATH ] . '/content/_update_by_query' )->setQueryString( array( 'conflicts' => 'proceed', 'wait_for_completion' => 'false' ) )->request()->setHeaders( array( 'Content-Type' => 'application/json' ) )->post( json_encode( array(
					'script'	=> array(
						'inline'	=> "ctx._source.index_hidden = " . intval( $newHiddenStatus ) . ";",
						'lang'		=> 'painless'
					),
					'query'		=> array(
						'bool'		=> $conditions
			...
			if ( $itemClass )
			{
				try
				{
					...
					if ( $lastComment = $item->comments( 1, 0, 'date', 'desc', NULL, FALSE ) )
					{
						...
						$this->url->setPath( $this->url->data[ \IPS\Http\Url::COMPONENT_PATH ] . '/content/_update_by_query' )->setQueryString( array( 'conflicts' => 'proceed', 'wait_for_completion' => 'false' ) )->request()->setHeaders( array( 'Content-Type' => 'application/json' ) )->post( json_encode( array(
							'script'	=> array(
								'inline'	=> "ctx._source.index_date_updated = " . intval( $indexDataForLastComment['index_date_updated'] ) ."; ctx._source.index_date_commented = " . intval( $indexDataForLastComment['index_date_commented'] ) .";",
								'lang'		=> 'painless'
							),
							'query'		=> array(
								'bool'		=> array(
