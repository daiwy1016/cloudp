...
{	
	...
	public function upload()
	{
		...
		if( \IPS\Settings::i()->calendar_venues_enabled )
		{
			$form->add( new \IPS\Helpers\Form\Node( 'feed_venue_id', NULL, TRUE, array(
				'url'					=> \IPS\Http\Url::internal( 'app=calendar&module=calendars&controller=ical&do=upload' ),
		...
		if ( $values = $form->values() )
		{
			...
			try
			{
				...
				$count	= $icsParser->parse( \file_get_contents( $values['feed_file'] ), $values['feed_calendar_id'], $values['feed_member_id'], ( isset( $values['feed_venue_id'] ) ) ? $values['feed_venue_id']->_id : NULL );
			}
