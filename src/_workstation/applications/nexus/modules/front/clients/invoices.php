...
class _invoices extends \IPS\Dispatcher\Controller
{
	...
	{	
		...
		if ( isset( \IPS\Request::i()->id ) )
		{
			...
			else if ( isset( \IPS\Request::i()->do ) and \IPS\Request::i()->do == 'printout' and isset( $_SESSION['nexusMember'] ) )
			{
				$this->invoice = \IPS\nexus\Invoice::load( \IPS\Request::i()->id );
				$guest = $this->invoice->guest_data;
				
				if ( $guest['member']['email'] != $_SESSION['nexusMember']['email'] )
				{
					\IPS\Output::i()->error( 'node_error', '2X215/5', 404, '' );
				}
			}
			else if ( isset( \IPS\Request::i()->do ) and \IPS\Request::i()->do == 'printout' and isset( \IPS\Request::i()->key ) )
			{
				$this->invoice = \IPS\nexus\Invoice::load( \IPS\Request::i()->id );
				$guest = $this->invoice->guest_data;

				if ( md5( $guest['member']['email'] . ';' . $guest['member']['joined'] ) != \IPS\Request::i()->key )
				{
