...
class _advanced extends \IPS\Dispatcher\Controller
{
	...
	protected function downloadDatastoreConstants()
	{
		...
		foreach ( \IPS\IPS::defaultConstants() as $k => $v )
		{
			...
			if ( $val !== $v and !in_array( $k, array( 'STORE_METHOD', 'STORE_CONFIG', 'CACHE_METHOD', 'CACHE_CONFIG', 'CACHE_PAGE_TIMEOUT', 'SUITE_UNIQUE_KEY', 'READ_WRITE_SEPARATION', 'REDIS_ENABLED', 'REPORT_EXCEPTIONS' ) ) )
			{
