		...
		if ( count( $matches ) )
		{
			if ( isset( $matches[1] ) )
			{
				...
				foreach( $mentions AS $member_name => $member_id )
				{
					...
					$post = preg_replace( "#\[user(=\"?(\d+)\"?)?\]@?{$memberNameEscaped}\[\/user\]#i", "<a contenteditable=\"false\" rel=\"\" href=\"{$urls[ $member_name ]}\" data-mentionid=\"{$maps[ $member_name ]}\" data-ipshover-target=\"{$cardUrls[ $member_name ]}\" data-ipshover=\"\">@{$member_name}</a>", $post );
				}
			}
		}
